...
export default function MobileNav({ role }: MobileNavProps) {
  ...
  const sidebarContent = (
    <div className="md:hidden">
      ...
      {/* Sidebar Panel */}
      <div className={`fixed top-0 left-0 h-[100dvh] w-72 bg-[#f7f4ed] text-[#1D1D1F] z-[9999] transform transition-transform duration-300 ease-in-out flex flex-col shadow-2xl ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          ...
          {role === "OWNER" && (
            <>
              ...
              <Link href="/dashboard/vehicles" onClick={closeNav} className="flex items-center px-4 py-3 rounded-xl text-slate-600 hover:bg-amber-50 hover:text-amber-700 transition-all font-medium">
                🚗 ยานพาหนะ / ที่จอดรถ
              </Link>
            </>
          )}
          ...
          {role === "TENANT" && (
            <>
              ...
              <Link href="/dashboard/my-meters/report" onClick={closeNav} className="flex items-center px-4 py-3 rounded-xl text-slate-600 hover:bg-amber-50 hover:text-amber-700 transition-all font-medium">
                รายงานมิเตอร์ของฉัน
              </Link>
            </>
          )}
