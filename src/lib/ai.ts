...
export async function draftVacancyListing(params: {
  ...
  const text = await generateText(
    `คุณเป็นนักเขียนโฆษณาอสังหาฯ มืออาชีพ เขียนโพสต์ประกาศปล่อยเช่าห้องพักลง Facebook/LINE ภาษาไทย ที่อ่านแล้วอยากทักแชทจองทันที
...
แนวการเขียน:
1. เปิดด้วยพาดหัวสั้น กระชับ สะดุดตา 1 บรรทัด (เช่น ชูจุดเด่นเรื่องราคา ทำเล หรือความพร้อมเข้าอยู่)
2. ตามด้วยรายละเอียดห้องและจุดเด่นเป็นรายการสั้น ๆ ขึ้นต้นแต่ละบรรทัดด้วยเครื่องหมายขีด "- "
3. บอกราคาให้เด่นชัด 1 บรรทัด
4. ปิดท้ายด้วยประโยคชวนตัดสินใจ + ช่องทางติดต่อ (ทักแชทเพจ หรือติดต่อผู้ดูแลหอพัก)
ใช้ภาษาเป็นกันเอง น่าเชื่อถือ ไม่โอเวอร์ ความยาวรวมไม่เกิน 12 บรรทัด

ข้อห้ามเด็ดขาด:
- ห้ามใช้ Markdown ทุกชนิด เช่น ** หรือ * หรือ # หรือ _ (ต้องเป็นข้อความธรรมดาล้วน เพราะจะนำไปวางในโพสต์ตรง ๆ)
- ห้ามใช้อีโมจิ (Emoji) หรือสัญลักษณ์พิเศษอื่นใด (อนุญาตเฉพาะเครื่องหมายขีด "-" หน้ารายการ)
- ห้ามแต่งข้อมูลที่ไม่ได้ให้มา เช่น ขนาดห้อง สถานีรถไฟฟ้า หากไม่มีในข้อมูล
ตอบเฉพาะข้อความประกาศเท่านั้น ไม่ต้องมีคำอธิบายนำหรือปิดท้าย`,
    600
  );

  if (!text) return null;
  // กันเหนียว: ลอก Markdown ออกถ้าโมเดลเผลอใส่มา (** * # _ ที่ใช้จัดรูปแบบ)
  const plain = text
    .replace(/\*\*([^*]+)\*\*/g, "$1")
    .replace(/\*([^*\n]+)\*/g, "$1")
    .replace(/\*/g, "")
